...
private:
	typedef function<bool(void* hFile, bool bFolder, const string& strPath, uint64_t uncompressedSize)> enum_zip_items_callback;

private:
	static bool _EnumZipItems(const char* zip_file, enum_zip_items_callback callback);
	static bool _ReadFileFromZip(void* hZip, const string& strPath, const string& strRootFolder, uint64_t uncompressedSize);
