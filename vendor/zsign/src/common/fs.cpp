...
map<void*, ZFile::MappedFile> ZFile::s_mapFiles;
#else
...
void* ZFile::MapFile(const char* path, size_t offset, size_t size, size_t* psize, bool ro)
{
	...
	FILE* fp = NULL;
	_fopen64(fp, path, ro ? "rb" : "r+b");
	if (NULL == fp && !ro) {
		_fopen64(fp, path, "w+b");
	}
	if (NULL != fp) {
		if (size <= 0) {
			struct stat st = { 0 };
		...
		base = malloc(size > 0 ? size : 1);
		if (NULL != base) {
			_fseeki64(fp, (off_t)offset, SEEK_SET);
			size_t readed = fread(base, 1, size, fp);
			if (readed == size) {
				s_mapFiles[base] = { path, size, ro };
			} else {
				free(base);
				base = NULL;
			}
...
bool ZFile::UnmapFile(void* base, size_t size)
{
	...
	bool ret = true;
	if (!it->second.ro) {
		ret = WriteFile(it->second.path.c_str(), (const char*)base, it->second.size);
	}
	free(base);
	s_mapFiles.erase(it);
	return ret;
